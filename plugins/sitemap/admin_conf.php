...
$dirplug = str_replace('../../','',e_PLUGIN."sitemap/");
...
switch ($action) 
{
	default:
		...
		<table style='".ADMIN_WIDTH."' class='fborder'>
			...
			<tr>
				...
				<td class='forumheader3' style='width:60%'>
					".$rs -> form_text("ITEM_PER_SITEMAP", 6, $sitemap_pref['ITEM_PER_SITEMAP'], 6  )." By default is 50000
				</td>
			</tr>			
			...
			<tr>
				...
				<td class='forumheader3' style='width:60%'>
					...
					$text.= $rs -> form_select_close()."						
					<br />". SITEMAP_MENU_L40 ." 
					".$rs -> form_select_open('sitemap_newscat_prio')."\n";
						for ($i=0.1; $i<1.0; $i=$i+0.1) {
							$sel = ($sitemap_pref['sitemap_newscat_prio'] == number_format($i,1))? "selected='selected'" : "";
							//$text .= "<option value='".number_format($i,1)."' $sel>".number_format($i,1)."</option>\n";
							$text .= $rs -> form_option(number_format($i,1), $sel, number_format($i,1))."\n";
						}
					$text.= $rs -> form_select_close()."						
	...
	case 'new':
		if($sitemap_pref['sitemap_active'] == 1)
		{
			if(!is_object($sql)){ $sql = new db; }
			$sitemap = new Sitemap();  
			$sitemap->startSitemapindex();				
			
			$text = "<div class='info'>" .SITEMAP_MENU_L13 .  "<a href='". $e107->http_path ."sitemap.xml' > ". $e107->http_path ."sitemap.xml </a></div><br />";
			
			// sitemap fro news 
			if ($sitemap_pref['sitemap_news'] == 1)			
				{					 
					$sitemap = new Sitemap();  
					$sitemap->setDomain($e107->http_path);
					$sitemap->setPath('data/');	
					$sitemap->setFilename('news');
					$div_style="success";
					$result = $sql->db_Select('news', '	news_id,news_title,news_datestamp', 			
						...
						while($row = $sql->db_Fetch()) {		
							$url = make_url_sitemap($row);
							$sitemap->addItem($url , $sitemap_pref['sitemap_news_prio'], $sitemap_pref['sitemap_news_freq'], $row[1],$sitemap_pref['ITEM_PER_SITEMAP'] );
						}	
						
					$message = SITEMAP_MENU_L3 . SITEMAP_MENU_L2;
					$sitemap->endIndex();
					$sitemap->createSitemapIndex1($e107->http_path . $dirplug , 'Today');
				}
			else 
				{
					$div_style="error";
			...
			if ($sitemap_pref['sitemap_newscat'] == 1)			
				{
					...
					$sitemap->setDomain($e107->http_path);
					$sitemap->setPath('data/');					
					$sitemap->setFilename('newscat');
					$div_style="success";
								
					$result = $sql->db_Select('news_category', 'category_id');
					//$result = $sql->db_Select('news_category', 'category_id,category_name');	
						
							while($row = $sql->db_Fetch()) 
							{
								//$catname = strtolower(ereg_replace(' ', '-', $row['category_name'])); //added seo
								//$sitemap->addItem('cat-' . $row[0] .'/'.strtolower($catname).'.html' , $sitemap_pref['sitemap_newscat_prio'], $sitemap_pref['sitemap_newscat_freq'],$sitemap_pref['ITEM_PER_SITEMAP']);
								$sitemap->addItem('news.php?cat.' . $row[0] , $sitemap_pref['sitemap_newscat_prio'], $sitemap_pref['sitemap_newscat_freq'],$sitemap_pref['ITEM_PER_SITEMAP']);
							}	
			...
			if ($sitemap_pref['sitemap_downloads'] == 1)
				{
					...
					$result = $sql->db_Select('download', '	download_id,download_datestamp', 
							...
							while($row = $sql->db_Fetch()) 
							{
								$sitemap->addItem('download.php?view.' . $row[0] , $sitemap_pref['sitemap_downloads_prio'], $sitemap_pref['sitemap_downloads_freq'], $row[1],$sitemap_pref['ITEM_PER_SITEMAP']);
							}	
			...
			if ($sitemap_pref['sitemap_downloadscat'] == 1)			
				{
					...
					$sitemap->setDomain($e107->http_path);
					$sitemap->setPath('data/');				
					$sitemap->setFilename('downloadscat');
					$div_style="success";
					...
					$result = $sql->db_Select('download_category', 'download_category_id', 
							...
							while($row = $sql->db_Fetch()) 
							{
								$sitemap->addItem('download.php?list.' . $row[0] , $sitemap_pref['sitemap_downloadscat_prio'], $sitemap_pref['sitemap_downloadscat_freq'],$sitemap_pref['ITEM_PER_SITEMAP']);
							}	
			...
			if ($sitemap_pref['sitemap_spiders'] == 1)
			{
				...
				$ping = "<div class='info'>";
				$ping .= "Google ping: " .$result[0][message]  ."<br />" ;
				$ping .= "Ask ping: " .$result[1][message]  ."<br />" ;
				$ping .= "Bing ping: " .$result[2][message]  ."<br />" ;
				$ping .= "</div>";
				$text .= $ping ;
			}
