...
class Sitemap {
	...
	const indexfilename = 'sitemap'; // added by mohamed anouar achoukhy
	 ...
	 * @return Sitemap
	 */
	public function addItem($loc, $priority = self::DEFAULT_PRIORITY, $changefreq = NULL, $lastmod = NULL, $ITEM_PER_SITEMAP = self::ITEM_PER_SITEMAP) {
		if (($this->getCurrentItem() % $ITEM_PER_SITEMAP) == 0) {
	...
	public function createSitemapIndex($loc, $lastmod = 'Today') {
		...
		$indexwriter->openURI($this->getPath() . $filename . self::SEPERATOR . self::INDEX_SUFFIX . self::EXT);
	...
	/////////****** MY OWN SITEMAP GENERATOR CLASSE *****////////
	public function endIndex() {
		$this->endSitemap();
	}

	public function startSitemapindex() {
		$sm_filename = "../../" . self::indexfilename . self::EXT ;
		$pf = fopen($sm_filename, 'w');	
	   		$smcontent = 
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'; 
	...
	public function createSitemapIndex1($loc, $lastmod = 'Today') {
		$sm_filename = "../../" . self::indexfilename . self::EXT ;
		$pf = fopen($sm_filename, 'a');
		
		for ($index = 0; $index < $this->getCurrentSitemap(); $index++) {
		$filecont ="
		<sitemap>
				<loc>".$loc . $this->getPath() . $this->getFilename() . self::SEPERATOR . $index . self::EXT."</loc>
				<lastmod>".$this->getLastModifiedDate($lastmod)."</lastmod>
		</sitemap>";
		fwrite($pf, $filecont);
		}	    
	    fclose($pf);
	    unset($this->current_sitemap);	
	}	

	public function endSitemapindex() {
		$sm_filename = "../../" . self::indexfilename . self::EXT ;
		$pf = fopen($sm_filename, 'a');	
	   		$smcontent = "\n</sitemapindex>"; 
		fwrite($pf, $smcontent);		   
	    fclose($pf);
		//@chmod($sm_filename, 0666);	
	}
	
    public function submitSitemap($yahooAppId = null) {
		$sitemapFullURL = $this->domain . self::indexfilename . self::EXT ;
