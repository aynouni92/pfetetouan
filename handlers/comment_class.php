...
class comment {
	...
	function form_comment($action, $table, $id, $subject, $content_type, $return = FALSE, $rating = FALSE, $tablerender = TRUE)
	{
		...
		if ($this->getCommentPermissions() == 'rw')
		{
			...
			$text = "\n<div style='text-align:center'><form method='post' action='".e_SELF."?".e_QUERY."' id='dataform'  onsubmit=\"return checkform(this);\" >\n<table style='width:100%'>";
			...
			<span id='txtCaptchaDiv' style='color:#F00'>".ADD_COMMENT_CAPTCHA." :</span> <input type='hidden' name='rand_num' value='".$sec_img->random_number."' />".$sec_img->r_image()."
			<input type='text' name='code_verify' size='15' maxlength='20' />
	...
	function enter_comment($author_name, $comment, $table, $id, $pid, $subject, $rateindex = FALSE)
	{
		...
		global $sql, $sql2, $tp, $e107cache, $e_event, $e107, $rater, $pref, $sec_img;
